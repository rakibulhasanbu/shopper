...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant, size, asChild = false, type = "button", href, isLoading, linkClassName, ...props }, ref) => {
        const Comp = asChild ? Slot : "button";
        const buttonContent = <>{isLoading ? <Loader2 className="size-4 animate-spin" /> : props.children}</>;

        if (href) {
            return (
                <Link className={cn("block", linkClassName)} href={props.disabled ? "" : href} passHref>
