    ...
    return (
        <HoverCard openDelay={200}>
            <HoverCardTrigger className="relative">
                <ShoppingBag className="size-5 cursor-pointer hover:text-primary" />
                <span className="absolute -right-2 -top-2 flex aspect-square size-4 items-center justify-center rounded-full bg-primary text-center text-xs text-white">
                    {summary.itemCount}
                </span>
            </HoverCardTrigger>
            <HoverCardContent className="w-80 border-foreground/90 bg-foreground/90 px-0 text-white" align="end">
                <div className="flex flex-col gap-2">
                    <ScrollArea className="flex max-h-[40dvh] flex-col gap-2 px-4">
                        {cartItems.map((item) => (
                            <div key={`${item.productId}-${item.variantId}`} className="flex items-center gap-2 py-1">
                                <Image
                                    src={item.photoURL || ""}
                                    alt={item.name || ""}
                                    width={56}
                    ...
                    <div className="flex items-center justify-between px-4">
                        <p className="text-sm font-medium">Total Items: {summary.itemCount}</p>
                        <p className="text-sm font-medium">Total: ৳ {summary.total.toLocaleString()}</p>
                    </div>
                    <Separator className="my-2 bg-white/20" />
